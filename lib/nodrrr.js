	...
	this.forEach(function(el, idx, ary) {
		switch(fmt[idx]) {
			...
			case 'n':
				...
				buf[1] = el;
				// FIXME this should not simply be 0 but match the high part of the number
				buf[0] = 0;
				len += 2;
				break;
			default:
...
Nodrrr.prototype.build_notification = function(name, title, desc, prio, sticky) {
	// TODO needs to build Data format sting
	// FIXME needs to build packet correctly
...
Nodrrr.prototype.build_registration = function() {
	...
	ecc.update(buf);
	if ( this.pass ) ecc.update(this.pass);
	digest = ecc.digest(encoding='hex');
	// FIXME write does write as encoding string instead of just writing the hex chars ... DUMBASS! 
	packet.write(digest, packet.length, encoding='hex');
